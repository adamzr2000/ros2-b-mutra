...
class MemoryStorageBackend:
    ...
    def export_to_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            logging.debug(f"Storage data exported to {file_path}")
            return True
        except Exception as e:
            logging.error(f"Failed to export storage data to {file_path}: {str(e)}")
            return False
    ...
    def load_from_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            with self._lock:
                self._data = data
            logging.info(f"Storage data loaded from {file_path}")
            return True
        except Exception as e:
            logging.error(f"Failed to load storage data from {file_path}: {str(e)}")
            return False
    ...
    def _save_to_file_if_configured(self):
        ...
        with self._lock:
            ...
            if mod_count % 10 == 0:
                try:
                    logging.debug(f"Saving memory storage to {self._storage_file} (modification count: {mod_count})")
                    self.export_to_file(self._storage_file)
                except Exception as e:
                    logging.error(f"Failed to auto-save memory storage: {str(e)}")
