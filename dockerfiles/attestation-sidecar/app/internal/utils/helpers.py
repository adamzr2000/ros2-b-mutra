...
# #     if role == "prover":
# #         # --- Timestamps ---
# #         if "prover_start" in timestamps:
# #             att["t_prover_start"] = timestamps["prover_start"]
            
# #         if "evidence_sent" in timestamps:
# #             att["t_evidence_sent"] = timestamps["evidence_sent"]
            
# #         if "evidence_sent_confirmed" in timestamps:
# #             att["t_evidence_sent_confirmed"] = timestamps["evidence_sent_confirmed"]
            
# #         if "result_received" in timestamps:
# #             att["t_result_received"] = timestamps["result_received"]
        
# #         if "prover_finished" in timestamps:
# #             att["t_prover_finished"] = timestamps["prover_finished"]
        
# #         # --- Durations (Calculated) ---
        
# #         # 1. Blockchain Write Latency (Time to confirm transaction)
# #         if "evidence_sent" in timestamps and "evidence_sent_confirmed" in timestamps:
# #             att["dur_evidence_tx_ms"] = timestamps["evidence_sent_confirmed"] - timestamps["evidence_sent"]

# #         # 2. E2E Latency (From sending evidence to getting result)
# #         if "evidence_sent" in timestamps and "result_received" in timestamps:
# #             att["dur_prover_e2e_ms"] = timestamps["result_received"] - timestamps["evidence_sent"]

# #         # 3. Total Prover Lifecycle (Measuring -> Cleanup)
# #         if "prover_start" in timestamps and "prover_finished" in timestamps:
# #             att["dur_prover_total_ms"] = timestamps["prover_finished"] - timestamps["prover_start"]
...
# #     elif role == "verifier":
# #         # --- Metadata ---
# #         if "verification_result" in timestamps:
# #             att["meta_verification_result"] = timestamps["verification_result"]

# #         # --- Timestamps ---
# #         if "verifier_start" in timestamps:
# #             att["t_verifier_start"] = timestamps["verifier_start"]
...
# #         if "verify_compute_start" in timestamps:
# #             att["t_verify_compute_start"] = timestamps["verify_compute_start"]
            
# #         if "verify_compute_finished" in timestamps:
# #             att["t_verify_compute_finished"] = timestamps["verify_compute_finished"]
            
# #         if "result_sent" in timestamps:
# #             att["t_result_sent"] = timestamps["result_sent"]
            
# #         if "result_sent_confirmed" in timestamps:
# #             att["t_result_sent_confirmed"] = timestamps["result_sent_confirmed"]

# #         if "verifier_finished" in timestamps:
# #             att["t_verifier_finished"] = timestamps["verifier_finished"]

# #         # --- Durations (Calculated) ---
        
# #         # 1. Fetch Latency (Reading from Blockchain)
# #         if "get_signatures_start" in timestamps and "get_signatures_finished" in timestamps:
# #             att["dur_signatures_fetch_ms"] = timestamps["get_signatures_finished"] - timestamps["get_signatures_start"]
            
# #         # 2. Compute Latency (Verification Logic)
# #         if "verify_compute_start" in timestamps and "verify_compute_finished" in timestamps:
# #             att["dur_verify_compute_ms"] = timestamps["verify_compute_finished"] - timestamps["verify_compute_start"]
            
# #         # 3. Blockchain Write Latency (Time to confirm result TX)
# #         if "result_sent" in timestamps and "result_sent_confirmed" in timestamps:
# #             att["dur_result_tx_ms"] = timestamps["result_sent_confirmed"] - timestamps["result_sent"]

# #         # 4. Total Verifier Lifecycle
# #         if "verifier_start" in timestamps and "verifier_finished" in timestamps:
# #             att["dur_verifier_total_ms"] = timestamps["verifier_finished"] - timestamps["verifier_start"]
...
# #     elif role == "oracle":
# #         # --- Timestamps ---
# #         if "oracle_start" in timestamps:
# #             att["t_oracle_start"] = timestamps["oracle_start"]
...
# #             att["t_get_prover_ref_signatures_db_start"] = timestamps["get_prover_ref_signatures_db_start"]
            
# #         if "get_prover_ref_signatures_db_finished" in timestamps:
# #             att["t_get_prover_ref_signatures_db_finished"] = timestamps["get_prover_ref_signatures_db_finished"]
            
# #         if "prover_ref_signatures_sent" in timestamps:
# #             att["t_prover_ref_signatures_sent"] = timestamps["prover_ref_signatures_sent"]    
            
# #         if "prover_ref_signatures_sent_confirmed" in timestamps:
# #             att["t_prover_ref_signatures_sent_confirmed"] = timestamps["prover_ref_signatures_sent_confirmed"]

# #         if "oracle_finished" in timestamps:
# #             att["t_oracle_finished"] = timestamps["oracle_finished"]

# #         # --- Durations (Calculated) ---

# #         # 1. Database/Lookup Latency
# #         if "get_prover_ref_signatures_db_start" in timestamps and "get_prover_ref_signatures_db_finished" in timestamps:
# #             att["dur_oracle_db_fetch_ms"] = timestamps["get_prover_ref_signatures_db_finished"] - timestamps["get_prover_ref_signatures_db_start"]

# #         # 2. Blockchain Write Latency
# #         if "prover_ref_signatures_sent" in timestamps and "prover_ref_signatures_sent_confirmed" in timestamps:
# #             att["dur_oracle_tx_ms"] = timestamps["prover_ref_signatures_sent_confirmed"] - timestamps["prover_ref_signatures_sent"]
            
# #         # 3. Total Oracle Lifecycle
# #         if "oracle_start" in timestamps and "oracle_finished" in timestamps:
# #             att["dur_oracle_total_ms"] = timestamps["oracle_finished"] - timestamps["oracle_start"]
    
# #     else:
# #         # Fallback or Error
# #         # info(f"Warning: Unknown role '{role}' encountered in export.")
# #         pass

# #     # 3. Atomic Write (Thread-Safe)
# #     now_ms = int(time.time() * 1000)
# #     with lock:
