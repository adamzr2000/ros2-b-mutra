...
def generate_figure(df, n_values, script_dir):
    ...
    out_path = script_dir / "barplot_attestation_time_v3.pdf"
...
def generate_csv(df, n_values, script_dir):
    ...
    out_path = script_dir / "attestation_time_breakdown_v3.csv"
...
def main():
    ...
    p_startup = (script_dir / INPUT_STARTUP).resolve()
    p_cont    = (script_dir / INPUT_CONTINUOUS.format(VARIANT=VARIANT)).resolve()

    frames = []
    for mode, path in [("startup", p_startup), ("continuous", p_cont)]:
        if not path.exists():
            print(f"[WARN] CSV not found, skipping {mode}: {path}")
            continue
