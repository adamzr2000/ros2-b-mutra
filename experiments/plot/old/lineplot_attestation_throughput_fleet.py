...
INPUT_FILE            = "../data/attestation-times/_summary/durations_raw_{VARIANT}.csv"
...
def main():
    ...
    required = {"n_robots", "role", "metric", "run"}
    missing  = required - set(df.columns)
    if missing:
        raise SystemExit(f"Missing columns: {missing}")

    # Total attestations per (n_robots, run) in continuous mode
    cpu_mask = df["cpu_limit"].isna() if CPU_LIMIT is None else (df["cpu_limit"] == CPU_LIMIT)
    sub = df[
        (df["ssp_ms"]  == SSP_MS) &
        (df["iterq"]   == ITERQ) &
        cpu_mask &
    ...
    ax.errorbar(n_vals, means, yerr=stds,
                ...
                markeredgecolor="none", linewidth=1.2, zorder=3,
