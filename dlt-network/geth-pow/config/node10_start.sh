...
# Define the command
command="geth --identity 'node10' --syncmode 'full' --nodiscover --ws --ws.addr $IP_NODE_10 --ws.port $WS_PORT_NODE_10 --datadir node10 --port $ETH_PORT_NODE_10 --bootnodes $BOOTNODE_URL --ws.api 'eth,net,web3,personal,miner,admin' --networkid $NETWORK_ID --nat 'any' --allow-insecure-unlock --authrpc.port $RPC_PORT_NODE_10 --ipcdisable --unlock $ETHERBASE_NODE_10 --password password.txt --mine --snapshot=false --miner.etherbase $ETHERBASE_NODE_10 --ethstats node10:$WS_SECRET@$ETH_NETSATS_IP:$ETH_NETSATS_PORT --miner.threads 1"  
